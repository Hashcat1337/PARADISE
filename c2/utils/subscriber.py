...
def viewuser():
    clear()
    users = "\n         • ".join(subscriptions.keys())
    user_count = len(subscriptions)
    users + ":"
    result = []
    
    max_username_length = max(len(username) for username in subscriptions.keys())
    formatted_data = [f"         • {username.ljust(max_username_length)}  >  {subscriptions[username]['plan']}\n" for username in subscriptions.keys()]
